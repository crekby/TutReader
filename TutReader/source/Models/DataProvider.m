...
- (void) checkWhichOnlineNewsIsFavorite
{
    [[PersistenceFacade instance] getNewsItemsListFromData:nil dataType:CORE_DATA_TYPE withCallback:^(NSMutableArray* data, NSError *error){
        ...
        if (requestResult) {
            ...
            [[NSNotificationCenter defaultCenter] postNotificationName:NEWS_TABLE_VIEW_REFRESH_TABLE_NOTIFICATION object:@(ONLINE)];
...
- (void)setupFavoriteNews
{
    [[PersistenceFacade instance] getNewsItemsListFromData:nil dataType:CORE_DATA_TYPE withCallback:^(NSMutableArray* data, NSError *error){
        ...
        if (requestResult) {
            ...
            [self setNews:array];
            [[NSNotificationCenter defaultCenter] postNotificationName:NEWS_TABLE_VIEW_REFRESH_TABLE_NOTIFICATION object:@(FAVORITE)];
            //[self performSelectorOnMainThread:@selector(reloadTableView) withObject:nil waitUntilDone:NO];
        }
    }];
}
